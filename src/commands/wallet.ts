...
export async function walletCommand(msg: Message) {
	try {
		...
		bot.sendMessage(
			msg.chat.id,
			`Your default wallet is ${defaultWallet.walletAddress} (Network: ${defaultWallet.network})\n\nYour balance is ${balance?.balance} ${balance?.symbol}\n\nWould you like to:`,
			options
		);
	} catch (error: any) {
...
export async function getAllWallets(chat_id: number) {
	try {
		...
		for (const wallet of wallets) {
			message += `🔹${wallet.walletAddress} (Network: ${wallet.network})\n`;
		}
	...
	try {
		...
		const messageHandler = async (msg: Message) => {
			...
			const wallet = wallets.find(
				(w: any) =>
					w.walletAddress === walletAddress && w.network === network
			);
